...
Mesh* MeshBuilder::GenerateText(const std::string &meshName, unsigned numRow, unsigned numCol, const char* textureFile) {
	...
	mesh->indexSize = index_buffer_data.size();
	mesh->mode = Mesh::DRAW_TRIANGLES;
	mesh->textureArray[0] = LoadTGA(textureFile);

	return mesh;

}

Mesh* MeshBuilder::GeneratePlane(const std::string &meshName, Color color, float length, unsigned int uTile, unsigned int vTile) {
	...
	v.pos.Set(-0.5f * length, 0.0f, 0.5f * length);
	v.texCoord.Set(0.0f, (float)vTile);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.0f, 0.5f * length);
	v.texCoord.Set((float)uTile, (float)vTile);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.0f, -0.5f * length);
	v.texCoord.Set((float)uTile, 0.0f);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.0f, -0.5f * length);
	v.texCoord.Set(0.0f, 0.0f);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::Generate2DTile(const string& meshName, Color color, float width, float height) {
