...
void PlayerSS::Update(const double& deltaTime) {
	...
	{
		/********************** MOVEMENT UPDATE WHEN NOT IN SUB SCENE *********************/
	...
	{
		if (InputManager::GetInstance().GetInputInfo().keyReleased[INPUT_JUMP])
		...
		case HOME:
			break;
		case COW:
			cout << "cow scene interact" << endl;
			break;
		case CHICKEN:
			cout << "chicken scene interact" << endl;
			break;
		case FISH:
			cout << "fish scene interact" << endl;
			break;
		case DRAGON:
			break;
		case WELL:
